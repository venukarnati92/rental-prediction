...
class TestMonitoringIntegration:
    ...
    def setup_method(self):
        ...
        self.current_data["bedrooms"] = self.current_data[
            "bedrooms"
        ] + np.random.choice([0, 1], 100, p=[0.8, 0.2])
        self.current_data["price"] = (
            self.current_data["price"] * 1.1
        )  # 10% price increase
    ...
    def test_evidently_data_drift_detection(self):
        ...
        assert drift_profile.drift_detected == (
            drift_metrics["drift_percentage"] > 0.05
        )
        ...
        class MockDataQualityProfile:
            ...
            def calculate_quality_metrics(self):
                ...
                self.quality_metrics = {
                    ...
                    "value_ranges": {
                        "bedrooms": {
                            "min": self.data["bedrooms"].min(),
                            "max": self.data["bedrooms"].max(),
                        },
                        "bathrooms": {
                            "min": self.data["bathrooms"].min(),
                            "max": self.data["bathrooms"].max(),
                        },
                        "square_feet": {
                            "min": self.data["square_feet"].min(),
                            "max": self.data["square_feet"].max(),
                        },
                        "price": {
    ...
    def test_evidently_target_drift_detection(self):
        ...
        class MockTargetDriftProfile:
            ...
            def calculate_target_drift(self):
                ...
                ref_price_dist = self.reference_data["price"].value_counts(
                    normalize=True
                )
                curr_price_dist = self.current_data["price"].value_counts(
                    normalize=True
                )
                ...
                if common_bins:
                    drift_score = sum(
                        abs(ref_price_dist.get(bin, 0) - curr_price_dist.get(bin, 0))
        ...
        target_drift_profile = MockTargetDriftProfile(
            self.reference_data, self.current_data
        )
                ...
                "panels": [
                    {
                        ...
                        "targets": [
                            {
                                "expr": (
                                    "SELECT drift_score FROM model_metrics "
                    ...
                    },
                    {
                        ...
                        "targets": [
                            {
                                "expr": (
                                    "SELECT timestamp, mae FROM model_metrics "
                                    "ORDER BY timestamp"
                                )
                            }
                        ],
    ...
    def test_metrics_collection_integration(self):
        ...
        class MockMetricsCollector:
            ...
            def collect_prediction_metrics(self, predictions, actual_values=None):
                ...
                if actual_values is None:
                    ...
                    actual_values = [
                        pred * 0.9 + np.random.normal(0, 50) for pred in predictions
                    ]

                # Calculate metrics
                mae = np.mean(np.abs(np.array(predictions) - np.array(actual_values)))
                rmse = np.sqrt(
                    np.mean((np.array(predictions) - np.array(actual_values)) ** 2)
                )
        ...
        quality_metrics = metrics_collector.collect_data_quality_metrics(
            self.current_data
        )
    ...
    def test_alerting_integration(self):
        ...
        performance_alerts = alerting_system.check_performance_alert(
            mae=350, r2_score=0.6
        )
