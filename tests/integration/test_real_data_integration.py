...
class TestRealDataIntegration:
    ...
    def test_real_data_processing_pipeline(self):
        ...
        def filter_data(df):
            ...
            df_filtered = df_filtered[
                (df_filtered["price"] > 500) & (df_filtered["price"] < 10000)
            ]
        ...
        assert len(filtered_data) > 0  # nosec B101
        ...
        # Verify new features
        assert "bed_bath_ratio" in engineered_data.columns  # nosec B101
        assert "sqft_x_beds" in engineered_data.columns  # nosec B101
        assert "price_per_sqft" in engineered_data.columns  # nosec B101

        # Verify data quality
        assert engineered_data["bed_bath_ratio"].dtype == "float64"  # nosec B101
        assert engineered_data["sqft_x_beds"].dtype == "float64"  # nosec B101
        assert engineered_data["price_per_sqft"].dtype == "float64"  # nosec B101

        # Verify no infinite values
        assert not np.isinf(engineered_data["bed_bath_ratio"]).any()  # nosec B101
        assert not np.isinf(engineered_data["sqft_x_beds"]).any()  # nosec B101
        assert not np.isinf(engineered_data["price_per_sqft"]).any()  # nosec B101

        # Verify realistic value ranges
        assert engineered_data["bed_bath_ratio"].min() > 0  # nosec B101
        assert engineered_data["bed_bath_ratio"].max() <= 5  # nosec B101
        assert engineered_data["price_per_sqft"].min() > 0  # nosec B101
        assert (
            engineered_data["price_per_sqft"].max() < 10
        )  # $10/sqft max  # nosec B101
    ...
    def test_real_model_training(self):
        ...
        filtered_data["bed_bath_ratio"] = filtered_data["bedrooms"] / filtered_data[
            "bathrooms"
        ].replace(0, 1)
        filtered_data["sqft_x_beds"] = (
            filtered_data["square_feet"] * filtered_data["bedrooms"]
        )
        filtered_data["price_per_sqft"] = (
            filtered_data["price"] / filtered_data["square_feet"]
        )
        ...
        assert len(predictions) == len(y_test)  # nosec B101
        assert all(pred > 0 for pred in predictions)  # nosec B101

        # With real data, the model should perform reasonably well
        assert mae < 1000  # MAE should be reasonable  # nosec B101
        assert rmse < 1500  # RMSE should be reasonable  # nosec B101
        assert r2 > 0.3  # R² should be positive and meaningful  # nosec B101

        # Verify predictions are reasonable
        assert (
            predictions.mean() > 1000
        )  # Average prediction should be realistic  # nosec B101
        assert (
            predictions.mean() < 8000
        )  # Average prediction should be realistic  # nosec B101
    ...
    def test_real_prediction_service(self):
        ...
        class RealPredictionService:
            ...
            def predict(self, features):
                ...
                # Apply feature engineering
                input_features["bed_bath_ratio"] = input_features[
                    "bedrooms"
                ] / input_features["bathrooms"].replace(0, 1)
                input_features["sqft_x_beds"] = (
                    input_features["square_feet"] * input_features["bedrooms"]
                )
                input_features[
                    "price_per_sqft"
                ] = 0  # Will be calculated after prediction

                # Make prediction
                prediction = self.model.predict(input_features[self.feature_columns])[0]

                # Calculate price per sqft
                input_features["price_per_sqft"] = (
                    prediction / input_features["square_feet"].iloc[0]
                )

                # Re-predict with updated price_per_sqft
                final_prediction = self.model.predict(
        ...
        for request in test_requests:
            ...
            assert prediction > 500  # nosec B101
            assert prediction < 10000  # nosec B101

        # Verify predictions are different (not all the same)
        assert len(set(predictions)) > 1  # nosec B101
    ...
    def test_real_data_validation(self):
        ...
        assert is_valid  # nosec B101
        assert message == "Valid data"  # nosec B101

        # Test invalid scenarios
        for i, invalid_data in enumerate(invalid_scenarios):
            is_valid, message = validate_data(invalid_data)
            assert not is_valid  # nosec B101
            assert "Invalid" in message  # nosec B101
    ...
    def test_real_performance_benchmarks(self):
        ...
        filtered_data = filtered_data[
            (filtered_data["price"] > 500) & (filtered_data["price"] < 10000)
        ]
        ...
        # Verify performance is reasonable (should complete in under 5 seconds for 5000 records)  # noqa: E501
        assert processing_time < 5.0  # nosec B101
        assert len(filtered_data) > 0  # nosec B101
        ...
        assert training_time < 10.0  # Should train in under 10 seconds  # nosec B101
        ...
        assert (
            prediction_time < 1.0
        )  # Should predict 100 samples in under 1 second  # noqa: E501
        assert len(predictions) == 100  # nosec B101
    ...
    def test_real_error_scenarios(self):
        ...
        def handle_missing_data(df):
            ...
            df_clean = df.dropna(
                subset=["bedrooms", "bathrooms", "square_feet", "price"]
            )
        ...
        assert len(cleaned_data) < len(data_with_missing)  # nosec B101
        assert missing_counts["bedrooms"] > 0  # nosec B101
        assert missing_counts["price"] > 0  # nosec B101
        ...
        assert len(cleaned_outliers) < len(data_with_outliers)  # nosec B101
        assert cleaned_outliers["price"].max() < 50000  # nosec B101
        assert cleaned_outliers["square_feet"].max() < 10000  # nosec B101
