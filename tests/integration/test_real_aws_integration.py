...
class TestRealAWSIntegration:
    ...
    def test_real_s3_operations(self):
        ...
        # Verify data
        assert (
            retrieved_data["model_version"] == test_data["model_version"]
        )  # nosec B101
        assert retrieved_data["features"] == test_data["features"]  # nosec B101
    ...
    def test_real_kinesis_operations(self):
        ...
        try:
            ...
            kinesis_client.create_stream(StreamName=self.test_stream, ShardCount=1)
        ...
        # Verify response
        assert "RecordId" in response  # nosec B101
        assert "ShardId" in response  # nosec B101
            ...
            "Records": [
                {
                    "kinesis": {
                        "data": base64.b64encode(
                            json.dumps(self.sample_data).encode()
                        ).decode()
                    }
                }
            ]
    ...
    def test_real_dynamodb_operations(self):
        ...
        response = table.get_item(
            Key={"run_id": "test-run-123", "timestamp": test_item["timestamp"]}
        )
    ...
    def test_real_cloudwatch_metrics(self):
        ...
        response = cloudwatch.put_metric_data(
            Namespace="RentalPrediction", MetricData=metric_data
        )
    ...
    def test_real_sqs_operations(self):
        ...
        response = sqs.send_message(
            QueueUrl=queue_url, MessageBody=json.dumps(test_message)
        )
        ...
        if "Messages" in response:
            ...
            # Verify message
            assert (
                received_data["prediction_request"]
                == test_message["prediction_request"]
            )
